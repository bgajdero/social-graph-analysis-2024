...
for location  in LOCATIONS:
    ...
    # clusters
    df_g = pd.read_csv('./data/graphs/node_groups_%s_2.csv'%(location))
...
df_g = []
for location  in LOCATIONS:
    _tweets = pd.read_csv('./data/graphs/sub_tweets_top_user_stats_%s_bias.csv'%(location), encoding = "iso-8859-1").rename(columns={'Unnamed: 0':'tweeter_username'})
    _tweets = _tweets[~pd.isnull(_tweets.prank)]
    _tweets['location'] = location
    _bias = pd.read_csv('./data/graphs/sub_tweets_top_user_stats_%s_bias.csv'%(location), encoding = "iso-8859-1").rename(columns={'Unnamed: 0':'tweeter_username'})
    _bias = _bias[~pd.isnull(_bias.prank)]


print("all")
print(pd.concat([stats_df, stats2_df]).sum())